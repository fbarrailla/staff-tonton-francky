...
import { COMMON_SKILLS, ROLE_LABEL } from '@/types'
...
export function EmployeeForm({
  ...
  function addSkill(s: string) {
    const v = s.trim()
    if (!v) return
    if (state.skills.includes(v)) return
    ...
    >
      ...
      <div className="flex items-center gap-5">
        ...
        <div className="space-y-1.5">
          <div className="text-[13px] text-ink-soft">Photo de profil</div>
          ...
          <p className="text-[11.5px] text-ink-faint">PNG ou JPEG, idéalement 400×400.</p>
        </div>
      </div>
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        ...
        />
        <Select label="Rôle" value={state.role} onChange={(e) => set('role', e.target.value as EmployeeRole)}>
        ...
        <Input
          ...
          type="date"
          value={state.hired_at}
          onChange={(e) => set('hired_at', e.target.value)}
        />
        <Select label="Statut" value={state.status} onChange={(e) => set('status', e.target.value as 'active' | 'inactive')}>
      ...
      <div>
        ...
        <div className="flex gap-2">
          ...
          />
          <Button type="button" variant="secondary" size="sm" iconLeft={<Plus size={13} />} onClick={() => addSkill(skillDraft)}>
            Ajouter
          </Button>
        </div>
      ...
      <Textarea
        ...
        rows={2}
        placeholder="Disponibilités particulières, mentions importantes…"
      />
