...
import {
  Plus, Search, Users, X, FileText, Link as LinkIcon, ArrowUpRight,
} from 'lucide-react'
...
export function Applicants() {
  ...
  const counts = useMemo(() => {
    const c: Record<StatusFilter, number> = { all: applicants.length, nouveau: 0, en_revue: 0, entretien: 0, embauche: 0, refuse: 0 }
    applicants.forEach((a) => { c[a.status]++ })
    return c
  }, [applicants])
    ...
    <Layout eyebrow={t('applicants.eyebrow')} title={t('applicants.title')}>
      ...
      <div className="flex flex-wrap items-center gap-3 mb-5">
        <div className="relative flex-1 min-w-[220px] max-w-md">
          ...
          <input value={q} onChange={(e) => setQ(e.target.value)}
            placeholder={t('applicants.search_ph')}
        ...
        <Tabs<StatusFilter> value={status} onChange={setStatus}
          items={[
            { value: 'all', label: t('applicants.tab_all'), count: counts.all },
            { value: 'nouveau', label: t('applicants.tab_new'), count: counts.nouveau },
            { value: 'en_revue', label: t('applicants.tab_in_review'), count: counts.en_revue },
            { value: 'entretien', label: t('applicants.tab_interview'), count: counts.entretien },
            { value: 'embauche', label: t('applicants.tab_hired'), count: counts.embauche },
            { value: 'refuse', label: t('applicants.tab_rejected'), count: counts.refuse },
          ]} />
      </div>

      <div className="flex flex-wrap gap-1.5 mb-6">
        <span className="text-[11.5px] text-ink-faint uppercase tracking-caps mr-1 self-center">{t('applicants.skills_label')}</span>
      ...
      {filtered.length === 0 ? (
        <EmptyState icon={<Users size={20} />}
          title={applicants.length === 0 ? t('applicants.empty_none_title') : t('applicants.empty_match_title')}
          description={applicants.length === 0 ? t('applicants.empty_none_desc') : t('applicants.empty_match_desc')}
          action={<Button variant="primary" onClick={() => setOpenAdd(true)}>{t('applicants.add')}</Button>} />
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
          {filtered.map((a) => (
            <Link key={a.id} to={`/candidats/${a.id}`}
              ...
              <div className="flex items-start gap-3.5">
                ...
                <div className="min-w-0 flex-1">
                  <div className="display text-[17px] leading-tight text-ink truncate">{a.full_name}</div>
                  <div className="text-[12.5px] text-ink-soft mt-0.5 truncate">{a.applied_position || t('applicants.position_unset')}</div>
                </div>
                <Badge tone={STATUS_TONE[a.status]} dot>{statusLabel(a.status)}</Badge>
              </div>
              ...
              {a.skills.length > 0 && (
                <div className="flex flex-wrap gap-1 mt-auto">
                  ...
                  {a.skills.length > 4 && (
                    <span className="text-[11px] text-ink-faint px-1.5 py-0.5">+{a.skills.length - 4}</span>
                  )}
                </div>
              )}
              <div className="flex items-center justify-between text-[11px] text-ink-faint border-t border-line pt-3 mt-1">
                <span>{t('applicants.received_on', { date: fmt(a.applied_at, 'd MMM yyyy') })}</span>
                <span className="flex items-center gap-2.5">
                  {a.cv_url && <span className="inline-flex items-center gap-0.5" title={t('applicants.cv_short')}><FileText size={11} /> {t('applicants.cv_short')}</span>}
                  {a.portfolio_url && <span className="inline-flex items-center gap-0.5" title={t('applicants.portfolio_short')}><LinkIcon size={11} /> {t('applicants.portfolio_short')}</span>}
                  <ArrowUpRight size={12} className="text-ink-faint group-hover:text-tonton-500 transition-colors" />
                </span>
              </div>
            </Link>
          ))}
      ...
      <Dialog open={openAdd} onClose={() => setOpenAdd(false)}
        title={t('applicants.new_application')} description={t('applicants.new_dialog_desc')} side="right">
        <ApplicantForm onSubmit={handleAdd} onCancel={() => setOpenAdd(false)}
