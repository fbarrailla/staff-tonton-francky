...
export function Dashboard() {
  ...
  async function approve(id: string) {
    try {
      ...
      toast.success(t('dashboard.approved_toast_title'), t('dashboard.approved_toast_body'))
    } catch (e) {
  ...
  async function reject(id: string) {
    try {
      await mutate.updateDayOff(id, {
        status: 'rejected',
        admin_note: t('dashboard.rejected_admin_note'),
      })
      toast.info(t('dashboard.rejected_toast_title'), t('dashboard.rejected_toast_body'))
    } catch (e) {
      toast.error(t('errors.action_failed'), formatError(e))
    }
  }

  const eyebrow = t('dashboard.eyebrow_today', {
    date: format(new Date(), 'EEEE d MMMM', { locale }),
  })
  const monthName = format(new Date(), 'MMMM yyyy', { locale })

  return (
    <Layout eyebrow={eyebrow} title={t('dashboard.greeting')}>
      <section className="grid lg:grid-cols-[1.4fr_1fr] gap-5 lg:gap-6 mb-8">
        <div className="surface-card p-7 lg:p-8 relative overflow-hidden">
          ...
          <div className="relative">
            <div className="label-caps mb-3">{t('dashboard.section_eyebrow_pre')} {monthName}</div>
            <p className="display text-[clamp(28px,3.2vw,42px)] leading-[1.05] text-ink text-balance max-w-[22ch]">
              <span className="text-tonton-600 dark:text-tonton-400">{working.length}</span>
              {t('dashboard.hero_in_post_pre')}{employees.length}{t('dashboard.hero_in_post_mid')}
              <span className="text-ink-soft">{t('dashboard.hero_off_count', { count: off.length })}</span>
              {sick.length > 0 && (
                <>, <span className="text-sick">{t('dashboard.hero_sick_count', { count: sick.length })}</span></>
              )}.
            </p>
            <p className="text-[14px] text-ink-soft mt-4 max-w-[52ch] leading-relaxed">
              {t('dashboard.hero_lead', { time: format(new Date(), 'HH:mm') })}
            </p>

            <div className="mt-6 flex flex-wrap items-center gap-2">
              <Button variant="primary" size="sm" iconLeft={<Plane size={14} />}>
                <Link to="/conges">{t('dashboard.see_pending_btn')}</Link>
              </Button>
              <Button variant="secondary" size="sm" iconLeft={<CalendarRange size={14} />}>
                <Link to="/calendrier">{t('dashboard.open_calendar_btn')}</Link>
              </Button>
            </div>
          </div>
        </div>
        ...
        <div className="surface-card p-6 flex flex-col">
          <div className="flex items-baseline justify-between">
            <div>
              <div className="label-caps">{t('dashboard.working_now')}</div>
          ...
          <div className="mt-5">
            ...
            <p className="text-[12.5px] text-ink-soft mt-3 leading-snug">
              {t('dashboard.working_caption', {
                count: working.length,
                names:
                  working.slice(0, 3).map((w) => w.e.full_name.split(' ')[0]).join(', ') +
                  (working.length > 3 ? t('common.and_others', { count: working.length - 3 }) : ''),
              })}
            </p>
          </div>
        </div>
      ...
      <section className="grid grid-cols-2 lg:grid-cols-12 gap-4 mb-10">
        <StatTile
          ...
          icon={<Users size={14} />}
          tone="neutral"
          label={t('dashboard.stat_total')}
          value={employees.length}
          hint={t('dashboard.stat_total_hint', { count: employees.filter(e => e.status === 'active').length })}
        />
        <StatTile
          className="lg:col-span-3"
          icon={<Plane size={14} />}
          tone="approved"
          label={t('dashboard.stat_off')}
          value={off.length}
          hint={off.length === 0 ? t('dashboard.stat_off_none') : t('dashboard.stat_off_some')}
        />
        <StatTile
          className="lg:col-span-3"
          icon={<Stethoscope size={14} />}
          tone="sick"
          label={t('dashboard.stat_sick')}
          value={sick.length}
          hint={sick.length === 0 ? t('dashboard.stat_sick_none') : t('dashboard.stat_sick_some')}
        />
        <StatTile
          className="lg:col-span-3"
          ...
          hint={t('dashboard.stat_balance_hint', { count: pending.length })}
        />
      </section>
      ...
      <section className="grid lg:grid-cols-[1.3fr_1fr] gap-6">
        <div>
          <header className="flex items-end justify-between mb-3">
            <div>
              <h2 className="display text-[22px] text-ink">{t('dashboard.to_validate')}</h2>
              <p className="text-[13px] text-ink-soft mt-0.5">
                {pending.length === 0
                  ? t('dashboard.no_pending')
                  : t('dashboard.pending_caption', { count: pending.length })}
              </p>
            </div>
            <Link
              to="/conges"
            ...
            >
              {t('common.see_all')} <ArrowUpRight size={13} />
            </Link>
          </header>
          ...
          {pending.length === 0 ? (
            <div className="surface-card p-8 text-center">
              ...
              <p className="text-[13.5px] text-ink-soft">{t('dashboard.all_clear_title')}</p>
            </div>
          ) : (
            <div className="surface-card divide-y divide-line overflow-hidden">
              {pending.slice(0, 5).map((d) => {
                ...
                const start = fmt(d.start_date, 'd MMM')
                const end = fmt(d.end_date, 'd MMM')
                return (
                  <div key={d.id} className="p-4 flex flex-wrap items-start gap-4 hover:bg-surface/60 transition-colors">
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        <Badge tone="pending" dot>{t('dashboard.leave_pending_badge')}</Badge>
                      </div>
                      <div className="text-[12.5px] text-ink-soft mt-1 tabular">
                        {start === end ? start : `${start} → ${end}`}
                        <span className="text-ink-faint"> · {t('common.day', { count: days })}</span>
                      </div>
        ...
        <div className="space-y-6">
          <div>
            <header className="flex items-end justify-between mb-3">
              <div>
                <h2 className="display text-[22px] text-ink">{t('dashboard.upcoming')}</h2>
                <p className="text-[13px] text-ink-soft mt-0.5">{t('dashboard.upcoming_subtitle')}</p>
              </div>
              <Link
                to="/calendrier"
                className="text-[12.5px] text-ink-soft hover:text-ink flex items-center gap-1"
              >
                {t('dashboard.see_calendar')} <ArrowUpRight size={13} />
              </Link>
            </header>
            {upcoming.length === 0 ? (
                    ...
                    <li key={item.id} className="p-3.5 flex items-center gap-3">
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-1.5">
                          ...
                          {item.type === 'sick' ? (
                            <Badge tone="sick" dot>{t('dashboard.sick_badge')}</Badge>
                          ) : item.status === 'pending' ? (
                            <Badge tone="pending">{t('dashboard.leave_pending_badge')}</Badge>
                          ) : (
                            <Badge tone="approved">{t('dashboard.leave_badge')}</Badge>
                          )}
                        </div>
          ...
          <div>
            <header className="flex items-end justify-between mb-3">
              <div>
                <h2 className="display text-[22px] text-ink">{t('dashboard.monthly_balances')}</h2>
                <p className="text-[13px] text-ink-soft mt-0.5">
                  {t('dashboard.monthly_balances_subtitle')}
                </p>
              </div>
            </header>
